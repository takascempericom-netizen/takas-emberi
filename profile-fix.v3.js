...
const normTitle = s => (s||"").toString().toLowerCase().replace(/\s+/g," ").trim();
...
async function firstPhotoUrl(data){
  ...
  const cover=data?.cover||data?.thumbnail||data?.thumb; if(cover) lists.unshift(cover);
...
async function fixByDocId(id,card){
  ...
  try{
    ...
    log(ok?"✓ id ile düzeltildi":"kart img yok",id,url);
    return ok;
...
async function fetchOwnerListings(){
  ...
  try{
    const qy=query(collection(db,"listings"), where("ownerId","==",u.uid), orderBy("createdAt","desc"), limit(200));
...
function collectCards(root=document){
  const cards=[];
  // data-* ve link
  const add=(id,card)=>cards.push({id,el:card});
  $all("[data-doc-id],[data-id],[data-listing-id],[data-open-listing],[data-listing]",root).forEach(el=>{
    ...
    if(id && /^[\w-]{15,}$/.test(id)) add(id,findCardRoot(el));
  });
  $all("a[href]",root).forEach(a=>{
    const id=parseIdFromHref(a.getAttribute("href"));
    if(id) add(id,findCardRoot(a));
  });
  // YENİ: <img src="...picsum.photos/seed/<id>/...">
  $all("img[src*='picsum.photos/seed/']",root).forEach(img=>{
    const id=parseIdFromImgSrc(img.getAttribute("src"));
    if(id) add(id,findCardRoot(img));
  });
  // Genel kartları da topla (id yoksa sıra fallback için)
  const set=new Set(cards.map(x=>x.el));
  $all(".listing-card,.ilan-card,.ilan,.listing,.card,article,li,.item,.row,.box,.grid-item,[class*='ilan']").forEach(el=>{
    if(!set.has(el)) cards.push({id:null,el});
  });
  return cards;
}
...
async function fixAll(){
  ...
  // Sıra fallback: owner listings ile sıradan doldur
  if(noId.length){
    const mine=await fetchOwnerListings();
    for(let i=0;i<noId.length && i<mine.length;i++){
      const hit=mine[i];
      if(hit?.url){ const ok=applyImageToCard(noId[i].el,hit.url); if(ok) log("✓ sıra fallback",i,hit.id); }
    }
  }
}
