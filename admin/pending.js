...
function toast(msg, type='info'){
  ...
  t.textContent = msg;
  t.className = `adm-toast ${type}`; // CSS'i index.html tarafında tanımlayabilirsin
...
// --- UI bağlama (index.html beklenen elementler)
//
// <div id="pendingToolbar">
//   <input id="search" placeholder="Başlıkta ara"/>
//   <select id="fCat">...</select>
//   <select id="fCity">...</select>
//   <button id="btnRefresh">Yenile</button>
// </div>
// <div id="pendingList" class="cards"></div>
// <button id="btnMore">Daha Fazla</button>
// Modal inceleme için:
// <dialog id="dlgView">
//   <article>
...
//       <button id="dlgApprove" class="ok">Onayla</button>
//       <button id="dlgReject" class="danger">Reddet</button>
//     </div>
//   </article>
// </dialog>
...
async function loadMore(){
  ...
  // Basit istemci-side filtre (başlık/şehir/kategori) — ilk etapta
  const snap = await getDocs(q);
  if(snap.empty){ reachedEnd = true; moreBtn && (moreBtn.disabled = true); loading = false; return; }
...
async function approve(id){
  try{
    ...
    // UI'dan kaldır
    const card = [...(listEl?.children||[])].find(c=>c.innerHTML.includes(`approve`)&&c.innerHTML.includes(id)===false);
    resetAndLoad();
    dlg?.open && dlg.close();
