...
function humanDate(ts) {
  try {
    return new Date(ts?.seconds ? ts.seconds * 1000 : ts).toLocaleString("tr-TR");
  } catch {
    return "-";
  }
}
...
onAuthStateChanged(auth, (user) => {
  if (!user) {
    location.href = "/admin/login.html?next=pending.html";
    return;
  }
  if (user.email !== ADMIN_EMAIL) {
    toast("Yetkiniz yok", "danger");
...
function openDetail(item) {
  ...
  const photos = (item.photoURLs || [])
    .map((u) => `<img src="${u}" style="width:100px;height:100px;object-fit:cover;border-radius:8px;border:1px solid #262b3d"/>`)
    .join("");
...
async function approve(id) {
  try {
    ...
    toast("Onay hata: " + (e?.message || e), "danger");
  }
}
...
async function rejectFlow(id) {
  ...
  try {
    ...
    toast("Red hata: " + (e?.message || e), "danger");
  }
}
...
searchIn && searchIn.addEventListener("input", () => {
  if (!loading) { listEl.innerHTML = ""; lastDoc = null; reachedEnd = false; loadMore(); }
});
